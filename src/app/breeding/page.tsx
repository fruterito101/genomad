...
const traitIcons: Record<string, typeof Cpu> = { technical: Cpu, creativity: Palette, social: MessageSquare, analysis: Brain, empathy: Heart, trading: TrendingUp, teaching: GraduationCap, leadership: Crown };
            ...
            {parentA && parentB && (
              <motion.div className="mt-6 sm:mt-8 p-4 sm:p-6 rounded-xl bg-background" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                  {predictedTraits.map(({ trait, avg }) => { const Icon = traitIcons[trait as string]; const color = traitColors[trait as string]; return (
                    <div key={trait} className="p-2 sm:p-3 rounded-lg bg-card">
                      <div className="flex items-center gap-1 sm:gap-2 mb-1 sm:mb-2"><Icon className="w-3 h-3 sm:w-4 sm:h-4" color={color || "#888"} /><span className="text-[10px] sm:text-xs capitalize truncate text-muted-foreground">{trait}</span></div>
